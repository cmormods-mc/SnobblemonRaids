...
const isActivePlayerSide = side => isPlayerSide(side) && !side.raidWithdrawn;
...
Battle.prototype.makeRequest = function(type) {
  ...
  for (const side of this.sides) {
    if (!isPlayerSide(side) || !side.raidWithdrawn) continue;
    side.activeRequest = {wait: true, side: side.getRequestData()};
    side.clearChoice();
  }
...
Battle.prototype.sendUpdates = function() {
  if (isRaid(this)) this.sides.forEach(raidNormalizeSide);
...
module.exports = {isRaid, bossIndex, bossSide, isBossSide, isPlayerSide};
